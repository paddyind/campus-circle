...
| **Chrome works, app doesn't** | On Android, the app WebView blocks HTTP (cleartext) by default. The project sets both (1) `android:usesCleartextTraffic="true"` and (2) `android:networkSecurityConfig="@xml/network_security_config"` (with `res/xml/network_security_config.xml` allowing cleartext) so the app can reach `http://YOUR_IP:8000/api`. **You must install an APK built from this code** — old APKs won't have it. **Validate before installing:** In CI, the Android APK job has a step **"Verify built APK allows cleartext (HTTP)"** that checks the built APK's manifest; if that step is **green**, the uploaded artifact is safe to install. Download the APK only from a run where this step passed. |
